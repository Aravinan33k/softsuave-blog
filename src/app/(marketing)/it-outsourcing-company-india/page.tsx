...
const PAGE_CTA = { label: 'Scope your project', href: '#enquiry' } as const;

const structuredData = [
  {
    '@context': 'https://schema.org',
    '@type': 'FAQPage',
    mainEntity: itoFaqs.items.map((f) => ({
      '@type': 'Question',
      name: f.q,
      acceptedAnswer: {
        '@type': 'Answer',
  ...
  },
  {
    ...
    hasOfferCatalog: {
      '@type': 'OfferCatalog',
      name: itoServices.title,
      itemListElement: itoServices.items.map((i) => ({
        '@type': 'Offer',
...
export default function ItOutsourcingCompanyIndiaPage() {
  const trail = [
    ...
    { name: itoMeta.title, path: itoMeta.path },
  ];
