...
const PAGE_NAV = [
  ...
  { label: 'Core Tech', href: '#core-tech' },
...
const PAGE_CTA = { label: 'Get Free Estimate', href: '#enquiry' } as const;

/** FAQPage + Service structured data — this page's answers are its SEO surface. */
const faqLd = {
  '@context': 'https://schema.org',
  '@type': 'FAQPage',
  mainEntity: webFaqs.items.map((f) => ({
    '@type': 'Question',
    name: f.q,
    acceptedAnswer: {
      '@type': 'Answer',
...
const serviceLd = {
  ...
  hasOfferCatalog: {
    '@type': 'OfferCatalog',
    name: webServices.title,
    itemListElement: webServices.items.map((i) => ({
      '@type': 'Offer',
...
export default function WebApplicationDevelopmentPage() {
  ...
  const trail = [
    ...
    { name: 'Web Application Development', path: webMeta.path },
  ];
