  ...
  try {
    ...
    if (parsed.data.slug !== undefined || parsed.data.name !== undefined) {
      ...
      data.slug = await ensureUniqueSlug(
        base,
        async (s) => (await prisma.tag.count({ where: { slug: s, NOT: { id } } })) > 0,
      );
    }

    const tag = await prisma.tag.update({ where: { id }, data });
    await logAudit({ action: 'UPDATE', userId: session.sub, targetType: 'tag', targetId: id, req });
    return NextResponse.json({ tag });
  } catch (err) {
    return handleRouteError(err, 'admin/tags PATCH');
  }
}
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    await logAudit({ action: 'DELETE', userId: session.sub, targetType: 'tag', targetId: id, req });
    return NextResponse.json({ ok: true });
  } catch (err) {
    return handleRouteError(err, 'admin/tags DELETE');
  }
}
