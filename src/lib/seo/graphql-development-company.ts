 ...
 *
 * Same pattern as `nextjs-development-company.ts` and
 * `typescript-development-company.ts`: these reference `organizationLd`'s
 * `@id` for `provider`/`publisher` rather than repeating that object, which is
 * why the route also emits `organizationLd` itself alongside these three.
 *
